...
#[test]
fn test_request_refund_persists_each_reason_code() {
    let env = Env::default();
    let contract_id = env.register(RefundContract, ());
    let client = RefundContractClient::new(&env, &contract_id);
    let admin = Address::generate(&env);
    ...
    let customer = Address::generate(&env);
    let token = Address::generate(&env);
    let reason = String::from_str(&env, "coded reason");
    ...
    for (idx, code) in reason_codes.iter().enumerate() {
        ...
        let stored = client.get_refund(&refund_id);
        assert_eq!(stored.reason_code, code.clone());
    }
}

#[test]
fn test_reason_code_analytics_sorted_by_frequency() {
    let env = Env::default();
    let contract_id = env.register(RefundContract, ());
    let client = RefundContractClient::new(&env, &contract_id);
    let admin = Address::generate(&env);
    ...
    assert_eq!(
        ...
        (RefundReasonCode::ProductDefect, 3u64),
    );
    assert_eq!(
        analytics.get(1).unwrap(),
        (RefundReasonCode::CustomerRequest, 2u64),
    );
    assert_eq!(
        analytics.get(2).unwrap(),
        (RefundReasonCode::DuplicateCharge, 1u64),
    );
    assert_eq!(
        analytics.get(3).unwrap(),
        (RefundReasonCode::NonDelivery, 0u64),
    );
    assert_eq!(
        analytics.get(4).unwrap(),
...
#[test]
fn test_get_refunds_by_reason_code_with_pagination() {
    let env = Env::default();
    let contract_id = env.register(RefundContract, ());
    let client = RefundContractClient::new(&env, &contract_id);
    let admin = Address::generate(&env);
    ...
    let page = client.get_refunds_by_reason_code(&RefundReasonCode::NonDelivery, &2u64, &1u64);
    assert_eq!(page.len(), 2);
    ...
    assert_eq!(first.get(0).unwrap().id, r1);
}
